...
- 原始类型的本地变量，总是 “呆在” 线程栈上。
- 引用类型的本地变量，引用（本地变量）存放在线程栈上，但是对象本身放在堆上。
- 一个包含方法的对象，方法中包含本地变量，这些本地变量存在放线程栈上，即使这些方法所属的对象存放在堆上。
- 对象的成员变量随着对象自身存放在堆上。不管成员变量是原始类型还是引用类型。
- 静态成员变量存放在堆上。
- 存放在堆上的对象可以被所有持有对这个对象引用的线程访问。如果两个线程同时调用同一个对象上的同一个方法，每一个线程都拥有这个成员变量的私有拷贝。



<img src="/images/java/WX20230306-102130@2x.png" style="zoom:50%;" />



<img src="/images/java/WX20230306-103214@2x.png" style="zoom:50%;" />



现代计算机硬件架构的简单图示：

<img src="/images/java/WX20230306-104854@2x.png" style="zoom:50%;" />
...
<img src="/images/java/WX20230306-105552@2x.png" style="zoom:50%;" />



从抽象的角度来看，<font color=red>**JMM定义了线程和主内存之间的抽象关系**</font>：
...
<img src="/images/java/WX20230306-112509@2x.png" style="zoom:50%;" />
