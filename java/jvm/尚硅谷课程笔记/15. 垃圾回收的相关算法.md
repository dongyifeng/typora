...
![](/images/java/WX20221228-144714@2x.png)
...
![](/images/java/WX20221228-151803@2x.png)
...
如图 obj1.reference = null 和 obj2.reference = null 

![](/images/java/screenshot-20221228-165926.png)
...
![](/images/java/screenshot-20221228-172726.png)
...
![](/images/java/screenshot-20221228-193314.png)
...
![](/images/java/screenshot-20221228-194623.png)
...
![](/images/java/screenshot-20221228-221227.png)
...
![](/images/java/WX20221229-145244@2x.png)

![](/images/java/WX20221229-145359@2x.png)
...
![](/images/java/WX20221229-153121@2x.png)
...
**注意：**这里所谓的清除并不是真的置空，而是把需要清除的对象地址保存在空闲的地址列表里。下次有新对象需要加载时，判断垃圾的位置是否足够，如果够，就存放。
...
将活着的内存空间分为两份，每次只使用其中一块，在垃圾回收时将正在使用的内存中的存活对象复制到未被使用的内存块中，之后清除正在使用的内存块中的所有对象，交换两个内存的角色，最后完成垃圾回收。



如下图：内存块 A 区和 B 区交替使用。GC 后内存是连续的，适合指针碰撞。

![](/images/java/WX20221229-155014@2x.png)
...
如果系统中的垃圾对象很多，复制算法需要复制的对象数量不是太大，这种场景适合复制算法，比如 YGC 的 s0 和 s1
...
![](/images/java/WX20221229-173945@2x.png)
...
- 年轻代（Young Gen）
  ...
  - 年轻代使用复制算法回收整理，速度最快。复制算法的效率只和当前存活对象大小有关，因此很适合用于年轻代的回收。而复制算法内存利用率不高的问题，通过 hotspot 中的两个 survivor 的设计得到缓解。
- 老年代（Tenured Gen）
  - 老年代特点：区域较大，对生生命周期长、存活率高，回收不及年轻代频繁。
  - 这种情况存在大量存活率高的对象，复制算法明显变得不合适。一般由标记-清除算法或者标记-整理算法混合实现
